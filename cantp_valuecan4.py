...
bus = can.interface.Bus(interface='neovi', channel=1, bitrate=500000)
...
def SingleFrameHandle(msg: can.Message):
...
def FirstFrameHandle(msg: can.Message):
...
def ConsecutiveFrameHandle(msg: can.Message):
    global Receive_Info
...
def FlowControlHandle(msg: can.Message):
...
class ListenerHandlebus(can.Listener):
    def on_message_received(self, msg: can.Message):
        ...
        if msg.data[0] >> 4 == 0:
            print("Single frame")
            SingleFrameHandle(msg)
        elif msg.data[0] >> 4 == 1:
            print("First frame")
            FirstFrameHandle(msg)
        elif msg.data[0] >> 4 == 2:
            print("Consecutive frame")
            ConsecutiveFrameHandle(msg)
        elif msg.data[0] >> 4 == 3:
            print("Flow control")
            FlowControlHandle(msg)

def Transmit(id: int, TX_DL: int , data_buf: list, length: int, is_fd: bool = False):
    global Transmit_Info, bus, receive_fc
...
if __name__ == "__main__":
    ...
    try:
        while 1:
            choice = input()
            if choice == 's1':
                Transmit(id=0x012, TX_DL=8, data_buf=string_to_ascii_list(send_data_b), length=len(send_data_b))
            elif choice == 's1':
                Transmit(id=0x012, TX_DL=8, data_buf=string_to_ascii_list(send_data_a), length=len(send_data_a))
            else:
                print("quit")
                break
